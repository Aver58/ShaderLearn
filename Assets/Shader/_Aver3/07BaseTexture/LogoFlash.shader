...
Properties {
    ...
    _width ("_width", float) = 0.25
	_angle("_angle", float) = 75
	_speed("_speed", float) = 5
}
SubShader {
	Tags {
		"Queue"="Transparent" 
		...
		v2f vert (appdata_base v)
	{
		...
		float4 frag (v2f i) : COLOR
		{
			...
			//传进i.uv等参数，得到亮度值
                float tmpBrightness;
                tmpBrightness =inFlash(_angle,i.uv,_width,_speed,2,0.15,0.7);
			//图像区域，判定设置为 颜色的A > 0.5,输出为材质颜色+光亮值
		    if(texColor.w >0.5)
		            outp  =texColor+float4(1,1,1,1)*tmpBrightness;
