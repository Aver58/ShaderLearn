Shader "XTech/HeroShow"
{
	Properties
	{
		...
		_LightPos ("Light Position(方向光位置)", Vector) = (1,1,1,1)
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 evirCol = tex2D(_LightTex, i.uv.zw) * _LightPower * maskCol.r;

				fixed3 N = normalize(i.worldNormal).xyz;
				fixed3 L = normalize(i.worldPos.xyz - _LightPos.xyz);
				...
				fixed3 resultCol = (mainCol.rgb + evirCol.rgb + specular)  * _Bright;

				fixed3 shadowCol = fixed3(NdotL, NdotL, NdotL) * mainCol.rgb;
...
//				fixed3 resultCol = (mainCol.rgb + evirCol.rgb + specular)  * (NdotL * _Bright) ;
	
//				fixed3 luminanceColor = Luminance(resultCol);
//
//				resultCol = lerp(luminanceColor, resultCol, _Saturation);
//
//				fixed3 avgColor = fixed3(0.5, 0.5, 0.5);  
//                
//                resultCol = lerp(avgColor, resultCol, _Contrast);  


//				fixed4 maskCol = tex2D(_MaskTex, i.uv.xy);
//				fixed4 lightCol = tex2D(_LightTex, i.uv.zw) * _LightPower;
//
//				fixed3 N = normalize(i.worldNormal).xyz;
//				fixed3 L = normalize(i.worldPos - _LightPos);
//				fixed3 V = normalize(UnityWorldSpaceViewDir(i.worldPos));
//				fixed3 H = normalize(L + V);
//
//				float NdotH = saturate(dot(N, L));
//
//				fixed3 specular = _LightColor.rgb * mainCol * pow(NdotH, _Gloss) * _SpecularBright;
//
//				float heightOffset = i.worldPos.w + N.z + _HeightControl;
//
//				fixed3 heightCol = saturate(lerp(_HeightColor.rgb, fixed3(1,1,1), heightOffset));
//
//				fixed3 heroCol = mainCol.rgb + (specular + lightCol.rgb) * maskCol.r;
//
//				return fixed4(heightCol * heroCol * (NdotH + _Bright)  , 1);