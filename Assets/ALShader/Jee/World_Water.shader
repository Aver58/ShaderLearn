    ...
    SubShader{
		...
		Pass{
			...
			v2f Vert(VertexInput v){
				...
				o.worldViewDir = UnityWorldSpaceViewDir(worldPos);// mul(unity_ObjectToWorld, v.vertex);
			...
			fixed4 frag (v2f i) : COLOR{
				...
				fixed3 worldView = -normalize(i.worldViewDir);// -normalize(i.worldPos - _WorldSpaceCameraPos);   
				        
				fixed4 nmap1 = tex2D(_Water_01, TRANSFORM_TEX(i.uv.yx, _Water_01) + float2(_Time.x * _WaveSpeed, 0));
				fixed4 nmap2 = tex2D(_Water_02, TRANSFORM_TEX(i.uv.yx, _Water_02) - float2(_Time.x * _WaveSpeed, 0));
				...
				fixed4 finalCol = fixed4(color, 1);
